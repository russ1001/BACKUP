...
// import java utility using * wildcard
import java.util.*;
// declare PermutationsOfString class extending the main test class
public class PermutationsOfString extends TestRecursiveMethods{
	// declare main method
	public static void main(String[] args) {
		// start timer
		elapsedTimeStart = System.currentTimeMillis();
		// declare and initialize the Scanner variable input to the input console
		...
		System.out.println("Enter an upper limit for the length of the characters in the string: ");
		// assign the input from user to the upperBound variable
		upperBound = input.nextInt();
		// check upper limit size
		if(upperBound > 6) {
			System.out.println("ERROR: upper limit too large");
			TestRecursiveMethods.main(args);
		}
		// prompt user to enter string to have permutations displayed		
		System.out.println("Enter a string to be processed for permutations: ");
		String s = input.next();
		...
		// call displayPermutations recursive method
		displayPermutations(s);
		System.out.println("Total time was " + elapsedTimeTotal * -1 + " miliseconds");
		System.out.println("=====================================================");
		// increment counter for total recursive methods called
		countRecursiveCalls++;
		// return to main method of test class
		TestRecursiveMethods.main(args);
		
	}
	// recursive method for displaying permutations with a String being passed
	...
	public static void displayPermutations(String s1, String s2) {
		...
		if(n == 0) {
			...
			for(int i=0; i<n; i++) {
				// recursive call
				displayPermutations(s1 + s2.charAt(i), s2.substring(0,i) + s2.substring(i+1, n));
			}
		}
		// close if/else
	}
	// close helper method displayPermutations

}
// close PermutationsOfString class